...
class Discriminator(object):
    ...
    def _add_placeholders(self):
        ...
        self._target_mask = tf.placeholder(tf.float32, [hps.batch_size* hps.max_enc_sen_num, hps.max_enc_seq_len], name='target_mask')
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('discriminator'):
            ...
            with tf.variable_scope('embedding'):
                ...
                emb_dec_inputs = tf.nn.embedding_lookup(embedding, self._dec_batch)  # tensor with shape (batch_size, max_enc_steps, emb_size)
            ...
            with tf.variable_scope('output_projection'):
                w = tf.get_variable('w_output', [hps.hidden_dim, vsize], dtype=tf.float32, initializer=self.trunc_norm_init)
            ...
            with tf.variable_scope('decoder'):
                ...
                # tf.logging.info(tf.shape(emb_dec_inputs))
            ...
            decoder_outputs = tf.reshape(decoder_outputs,[-1, hps.hidden_dim])
            decoder_outputs = tf.nn.xw_plus_b(decoder_outputs, w, v)

            decoder_outputs = tf.reshape(decoder_outputs, [hps.batch_size * hps.max_enc_sen_num, hps.max_enc_seq_len,FLAGS.vocab_size])
    ...
    def run_pre_train_step(self, sess, batch):
        ...
        to_return = {
            ...
            'loss': self.train_loss,
            'out_loss': self.out_loss,
            'out_loss_sentence': self.out_loss_sentence,
            'global_step': self.global_step,
        }
